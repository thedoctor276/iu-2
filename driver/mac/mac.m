...
- (void)onMenuClick:(id)sender {
  ...
  onContextMenuClick((char *)[item.name UTF8String],
                     (char *)[item.compoID UTF8String]);
}
@end
...
void App_Quit() { defer([NSApp terminate:nil];); }

void App_SetBadge(const char *b) {
  NSString *badge = [NSString stringWithUTF8String:b];

  defer(NSDockTile *dock = [NSApp dockTile]; [dock setBadgeLabel:badge];);
}
...
NSMenu *Menu_GetOrSet(NSMenu *base, NSString *name) {
  ...
  if (item == nil) {
    item = [[NSMenuItem alloc] initWithTitle:name action:nil keyEquivalent:@""];
...
void Menu_SetMenuItem(NSMenu *nsmenu, Menu__ menu, NSString *title) {
  ...
  if (item == nil) {
    MenuItem *mitem =
        [[MenuItem alloc] initWithTitle:title action:nil keyEquivalent:@""];
    mitem.name = name;
...
- (instancetype)initWithID:(NSString *)ID andConf:(WindowConfig__)conf {
  ...
  NSUInteger styleMask = NSTitledWindowMask | NSFullSizeContentViewWindowMask |
...
- (void)windowDidMove:(NSNotification *)notification {
  ...
  onWindowMove((char *)[self.ID UTF8String], window.frame.origin.x,
               window.frame.origin.y);
}

- (void)windowDidResize:(NSNotification *)notification {
  NSWindow *window = (NSWindow *)notification.object;
  onWindowResize((char *)[self.ID UTF8String], window.frame.size.width,
...
- (void)windowWillClose:(NSNotification *)notification {
  [self.webView.configuration.userContentController
      removeScriptMessageHandlerForName:@"onCallEventHandler"];
  ...
  WindowController *windowController =
      (__bridge_transfer WindowController *)((__bridge void *)self);
}
...
void *Window_Create(const char *ID, WindowConfig__ conf) {
  ...
  WindowController *controller =
      [[WindowController alloc] initWithID:IDString andConf:conf];

  return (__bridge_retained void *)controller;
}

void Window_Show(void *ptr) {
  defer(WindowController *windowController = (__bridge WindowController *)ptr;
...
void Window_Move(void *ptr, CGFloat x, CGFloat y) {
  ...
  defer(CGPoint origin = windowController.window.frame.origin; origin.x = x;
        origin.y = y; [windowController.window setFrameOrigin:origin];);
}

void Window_Center(void *ptr) {
  WindowController *windowController = (__bridge WindowController *)ptr;

  defer([windowController.window center];);
}

void Window_Resize(void *ptr, CGFloat width, CGFloat height) {
  WindowController *windowController = (__bridge WindowController *)ptr;

  defer(CGRect frame = windowController.window.frame; frame.size.width = width;
...
void Window_Close(void *ptr) {
  ...
  defer([windowController.window performClose:windowController];);
}
...
void Window_Render(void *ptr, const char *HTML, const char *baseURL) {
  ...
  defer([windowController.webView loadData:html
                                  MIMEType:@"text/html"
...
void Window_CallJavascript(void *ptr, const char *c) {
  ...
  defer([windowController.webView evaluateJavaScript:call
...
void Window_ShowContextMenu(void *ptr, const Menu__ *menus, int count) {
  ...
  defer(NSPoint p = [windowController.window mouseLocationOutsideOfEventStream];
...
void Window_Alert(void *ptr, const char *msg) {
  ...
  defer(NSAlert *alert = [[NSAlert alloc] init]; [alert setMessageText:message];
